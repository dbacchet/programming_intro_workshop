#pragma once
/*
    #version:1# (machine generated, don't edit!)

    Generated by sokol-shdc (https://github.com/floooh/sokol-tools)

    Cmdline: sokol-shdc -i renderer-sapp.glsl -o renderer-sapp.glsl.h -l glsl330

    Overview:

        Shader program 'instancing':
            Get shader desc: instancing_shader_desc(sg_query_backend());
            Vertex shader: vs_instancing
                Attribute slots:
                    ATTR_vs_instancing_pos = 0
                    ATTR_vs_instancing_color0 = 1
                    ATTR_vs_instancing_inst_tf_c1 = 2
                    ATTR_vs_instancing_inst_tf_c2 = 3
                    ATTR_vs_instancing_inst_tf_c3 = 4
                    ATTR_vs_instancing_inst_tf_c4 = 5
                Uniform block 'vs_params':
                    C struct: vs_params_t
                    Bind slot: SLOT_vs_params = 0
            Fragment shader: fs_instancing

        Shader program 'vertexcolor':
            Get shader desc: vertexcolor_shader_desc(sg_query_backend());
            Vertex shader: vs_vertexcolor
                Attribute slots:
                    ATTR_vs_vertexcolor_pos = 0
                    ATTR_vs_vertexcolor_color0 = 1
                Uniform block 'vs_params':
                    C struct: vs_params_t
                    Bind slot: SLOT_vs_params = 0
            Fragment shader: fs_vertexcolor


    Shader descriptor structs:

        sg_shader instancing = sg_make_shader(instancing_shader_desc(sg_query_backend()));
        sg_shader vertexcolor = sg_make_shader(vertexcolor_shader_desc(sg_query_backend()));

    Vertex attribute locations for vertex shader 'vs_vertexcolor':

        sg_pipeline pip = sg_make_pipeline(&(sg_pipeline_desc){
            .layout = {
                .attrs = {
                    [ATTR_vs_vertexcolor_pos] = { ... },
                    [ATTR_vs_vertexcolor_color0] = { ... },
                },
            },
            ...});

    Vertex attribute locations for vertex shader 'vs_instancing':

        sg_pipeline pip = sg_make_pipeline(&(sg_pipeline_desc){
            .layout = {
                .attrs = {
                    [ATTR_vs_instancing_pos] = { ... },
                    [ATTR_vs_instancing_color0] = { ... },
                    [ATTR_vs_instancing_inst_tf_c1] = { ... },
                    [ATTR_vs_instancing_inst_tf_c2] = { ... },
                    [ATTR_vs_instancing_inst_tf_c3] = { ... },
                    [ATTR_vs_instancing_inst_tf_c4] = { ... },
                },
            },
            ...});

    Image bind slots, use as index in sg_bindings.vs_images[] or .fs_images[]


    Bind slot and C-struct for uniform block 'vs_params':

        vs_params_t vs_params = {
            .mvp = ...;
        };
        sg_apply_uniforms(SG_SHADERSTAGE_[VS|FS], SLOT_vs_params, &SG_RANGE(vs_params));

*/
#include <stdint.h>
#include <stdbool.h>
#if !defined(SOKOL_SHDC_ALIGN)
  #if defined(_MSC_VER)
    #define SOKOL_SHDC_ALIGN(a) __declspec(align(a))
  #else
    #define SOKOL_SHDC_ALIGN(a) __attribute__((aligned(a)))
  #endif
#endif
#define ATTR_vs_vertexcolor_pos (0)
#define ATTR_vs_vertexcolor_color0 (1)
#define ATTR_vs_instancing_pos (0)
#define ATTR_vs_instancing_color0 (1)
#define ATTR_vs_instancing_inst_tf_c1 (2)
#define ATTR_vs_instancing_inst_tf_c2 (3)
#define ATTR_vs_instancing_inst_tf_c3 (4)
#define ATTR_vs_instancing_inst_tf_c4 (5)
#define SLOT_vs_params (0)
#pragma pack(push,1)
SOKOL_SHDC_ALIGN(16) typedef struct vs_params_t {
    math::Matrix4f mvp;
} vs_params_t;
#pragma pack(pop)
/*
    #version 330
    
    uniform vec4 vs_params[4];
    layout(location = 0) in vec3 pos;
    out vec4 color;
    layout(location = 1) in vec4 color0;
    
    void main()
    {
        gl_Position = mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3]) * vec4(pos, 1.0);
        color = color0;
    }
    
*/
static const char vs_vertexcolor_source_glsl330[264] = {
    ,
};
/*
    #version 330
    
    layout(location = 0) out vec4 frag_color;
    in vec4 color;
    
    void main()
    {
        frag_color = color;
    }
    
*/
static const char fs_vertexcolor_source_glsl330[114] = {
    0x23,0x76,0x65,0x72,0x73,0x69,0x6f,0x6e,0x20,0x33,0x33,0x30,0x0a,0x0a,0x6c,0x61,
    0x79,0x6f,0x75,0x74,0x28,0x6c,0x6f,0x63,0x61,0x74,0x69,0x6f,0x6e,0x20,0x3d,0x20,
    0x30,0x29,0x20,0x6f,0x75,0x74,0x20,0x76,0x65,0x63,0x34,0x20,0x66,0x72,0x61,0x67,
    0x5f,0x63,0x6f,0x6c,0x6f,0x72,0x3b,0x0a,0x69,0x6e,0x20,0x76,0x65,0x63,0x34,0x20,
    0x63,0x6f,0x6c,0x6f,0x72,0x3b,0x0a,0x0a,0x76,0x6f,0x69,0x64,0x20,0x6d,0x61,0x69,
    0x6e,0x28,0x29,0x0a,0x7b,0x0a,0x20,0x20,0x20,0x20,0x66,0x72,0x61,0x67,0x5f,0x63,
    0x6f,0x6c,0x6f,0x72,0x20,0x3d,0x20,0x63,0x6f,0x6c,0x6f,0x72,0x3b,0x0a,0x7d,0x0a,
    0x0a,0x00,
};
/*
    #version 330
    
    uniform vec4 vs_params[4];
    layout(location = 0) in vec3 pos;
    layout(location = 2) in vec4 inst_tf_c1;
    layout(location = 3) in vec4 inst_tf_c2;
    layout(location = 4) in vec4 inst_tf_c3;
    layout(location = 5) in vec4 inst_tf_c4;
    out vec4 color;
    layout(location = 1) in vec4 color0;
    
    void main()
    {
        gl_Position = (mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3]) * mat4(inst_tf_c1, inst_tf_c2, inst_tf_c3, inst_tf_c4)) * vec4(pos, 1.0);
        color = color0;
    }
    
*/
static const char vs_instancing_source_glsl330[485] = {
    ,
};
/*
    #version 330
    
    layout(location = 0) out vec4 frag_color;
    in vec4 color;
    
    void main()
    {
        frag_color = color;
    }
    
*/
static const char fs_instancing_source_glsl330[114] = {
    0x23,0x76,0x65,0x72,0x73,0x69,0x6f,0x6e,0x20,0x33,0x33,0x30,0x0a,0x0a,0x6c,0x61,
    0x79,0x6f,0x75,0x74,0x28,0x6c,0x6f,0x63,0x61,0x74,0x69,0x6f,0x6e,0x20,0x3d,0x20,
    0x30,0x29,0x20,0x6f,0x75,0x74,0x20,0x76,0x65,0x63,0x34,0x20,0x66,0x72,0x61,0x67,
    0x5f,0x63,0x6f,0x6c,0x6f,0x72,0x3b,0x0a,0x69,0x6e,0x20,0x76,0x65,0x63,0x34,0x20,
    0x63,0x6f,0x6c,0x6f,0x72,0x3b,0x0a,0x0a,0x76,0x6f,0x69,0x64,0x20,0x6d,0x61,0x69,
    0x6e,0x28,0x29,0x0a,0x7b,0x0a,0x20,0x20,0x20,0x20,0x66,0x72,0x61,0x67,0x5f,0x63,
    0x6f,0x6c,0x6f,0x72,0x20,0x3d,0x20,0x63,0x6f,0x6c,0x6f,0x72,0x3b,0x0a,0x7d,0x0a,
    0x0a,0x00,
};
#if !defined(SOKOL_GFX_INCLUDED)
  #error "Please include sokol_gfx.h before renderer-sapp.glsl.h"
#endif
static inline const sg_shader_desc* instancing_shader_desc(sg_backend backend) {
  if (backend == SG_BACKEND_GLCORE33) {
    static sg_shader_desc desc;
    static bool valid;
    if (!valid) {
      valid = true;
      desc.attrs[0].name = "pos";
      desc.attrs[1].name = "color0";
      desc.attrs[2].name = "inst_tf_c1";
      desc.attrs[3].name = "inst_tf_c2";
      desc.attrs[4].name = "inst_tf_c3";
      desc.attrs[5].name = "inst_tf_c4";
      desc.vs.source = vs_instancing_source_glsl330;
      desc.vs.entry = "main";
      desc.vs.uniform_blocks[0].size = 64;
      desc.vs.uniform_blocks[0].uniforms[0].name = "vs_params";
      desc.vs.uniform_blocks[0].uniforms[0].type = SG_UNIFORMTYPE_FLOAT4;
      desc.vs.uniform_blocks[0].uniforms[0].array_count = 4;
      desc.fs.source = fs_instancing_source_glsl330;
      desc.fs.entry = "main";
      desc.label = "instancing_shader";
    };
    return &desc;
  }
  return 0;
}
static inline const sg_shader_desc* vertexcolor_shader_desc(sg_backend backend) {
  if (backend == SG_BACKEND_GLCORE33) {
    static sg_shader_desc desc;
    static bool valid;
    if (!valid) {
      valid = true;
      desc.attrs[0].name = "pos";
      desc.attrs[1].name = "color0";
      desc.vs.source = vs_vertexcolor_source_glsl330;
      desc.vs.entry = "main";
      desc.vs.uniform_blocks[0].size = 64;
      desc.vs.uniform_blocks[0].uniforms[0].name = "vs_params";
      desc.vs.uniform_blocks[0].uniforms[0].type = SG_UNIFORMTYPE_FLOAT4;
      desc.vs.uniform_blocks[0].uniforms[0].array_count = 4;
      desc.fs.source = fs_vertexcolor_source_glsl330;
      desc.fs.entry = "main";
      desc.label = "vertexcolor_shader";
    };
    return &desc;
  }
  return 0;
}
